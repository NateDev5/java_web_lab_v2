        ...
        return args -> {
            if (owners.count() == 0 && cars.count() == 0) {
                Owner alice = owners.save(
                        Owner.builder().firstName("Alice").lastName("Smith")
                                .email("alice@example.com").phone("+1-555-1111").build()
                );
                Owner bob = owners.save(
                        Owner.builder().firstName("Bob").lastName("Jones")
                                .email("bob@example.com").phone("+1-555-2222").build()
                );
                cars.save(Car.builder().brand("Ford").model("Mustang").color("Red")
                        .registrationNumber("ADF-1121").modelYear(2023).price(59000)
                        .owner(alice).build());
                cars.save(Car.builder().brand("Toyota").model("Prius").color("Silver")
                        .registrationNumber("KKO-0212").modelYear(2022).price(39000)
                        .owner(alice).build());
                cars.save(Car.builder().brand("Toyota").model("Corolla").color("Blue")
